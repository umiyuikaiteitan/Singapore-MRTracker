...
The Pages workflow builds the live train map at `/map/` automatically.
It combines calendar-aware trips from the original LTA GTFS feed with
OSM-reconstructed track shapes, then validates every train leg before
publishing. No layout variable is required.

The browser renders OSM tiles, station markers and continuously moving
train estimates. Fresh matching operator predictions adjust those estimates;
when predictions are unavailable or stale, the map uses the timetable and
labels the source. These are estimated positions, not measured vehicle locations.
The real-time refresh workflow runs every five minutes, subject to GitHub's
scheduling delays. The map polls its configured sources every 30 seconds.

All map assets and schedule data use relative URLs, so the generated
`site/map/` directory works under the repository's Pages path. A failed
map build or validation prevents publication of an incomplete site.
See [LIVE-MAP.md](LIVE-MAP.md) for the build commands, data matching rules
and freshness limits.

The Rust `mrt-map-static` and `mrt-map-web` schematic renderers remain
available as development prototypes. Their layout and snapshot settings
are separate from this production map.
