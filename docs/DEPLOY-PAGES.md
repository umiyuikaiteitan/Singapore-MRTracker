...
## The map site, separately

The live schematic map is not a section of this site. It is its own
static site, generated by `mrt-map-static` into its own output
directory (`index.html`, `.nojekyll`, `data/map.json`), and it
deploys to its own subdomain or repository — the board UI is not
touched by it. Deploying it works exactly like the board: a scheduled
workflow runs `cargo run --release -p mrt-map-static -- map-site
feed.zip` with the account key as a secret and publishes the
directory to Pages. The bundled `data/map.json` is only as fresh as
the last rebuild, so the Pages deployment is a schedule animation
with a delay hint, and the page says so; for second-level freshness,
run `mrt-map-web` on a server instead, or set `MRT_MAP_SNAPSHOT_URL`
at build time to a snapshot that a faster workflow refreshes.
