...
The Pages workflow can build it as a section at `/map/` after the board
and timetable steps. It is disabled until the repository Actions variable
`MRT_MAP_LAYOUT` names a committed, reviewed layout for the real network.
Do not use `config/layout-mini.geojson`: it contains fictional fixture
stations and branches and must not be combined with the production feed.
When configured, an invalid layout or failed map build fails the deployment
so that a broken map cannot silently replace the previous complete site.

The page's asset and data URLs are relative. The same output can also be
published as a separate Pages site or subdomain. `MRT_MAP_SNAPSHOT_URL` is
an optional Actions variable for a separately refreshed snapshot URL.
