...
test('parallel platform tracks choose a continuous alignment instead of a distant crossover',()=>{const ways=railData({elements:[way(1,[1,2],[[1.3,103.8],[1.3,103.9]]),way(2,[2,3],[[1.3,103.9],[1.3001,103.9]]),way(3,[3,4],[[1.3001,103.9],[1.3001,103.8]])]}).ways;const path=shapeThroughStops(ways,[[1.3,103.81],[1.3001,103.82],[1.3,103.83]]);assert.ok(path.every(p=>p[1]<103.84));assert.equal(path.callDistances.length,3);assert.ok(path.callDistances[2]<3000);});
test('emitted station distance markers stay in step with curved OSM shapes',()=>{const r=reconstruct(files,osm);assert.match(r.files['stop_times.txt'],/shape_dist_traveled/);assert.match(r.files['shapes.txt'],/shape_dist_traveled/);assert.equal(r.report.fallback,0);});
